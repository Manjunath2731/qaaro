...
import { fetchLamiOverviewDetails } from 'src/slices/ClientAdmin/SpPieChart';
...
interface SpOverviewDetailsProps {
    lamiId: string;
}

const SpPieChart: React.FC<SpOverviewDetailsProps> = ({ lamiId }) => {
    ...
    useEffect(() => {
        dispatch(fetchLamiOverviewDetails(lamiId));
    }, [dispatch, lamiId]);

    const LamiOverviewDetails = useSelector((state: RootState) => state.lamiOverviewDetails.details);

    const ensureNumber = (value: any) => typeof value === 'number' ? value : parseFloat(value) || 0;

    const total = ensureNumber(LamiOverviewDetails?.value?.total);
    const open = ensureNumber(LamiOverviewDetails?.value?.open);
    const success = ensureNumber(LamiOverviewDetails?.value?.success);
    const lost = ensureNumber(LamiOverviewDetails?.value?.lost);
    const returnedCount = ensureNumber(LamiOverviewDetails?.value?.returned);

    console.log("Values - Total:", total, "Open:", open, "Success:", success, "Lost:", lost, "Returned:", returnedCount);

    const openCount = ensureNumber(LamiOverviewDetails?.count?.open);
    const returned = ensureNumber(LamiOverviewDetails?.count?.returned);
    const customerAccepted = ensureNumber(LamiOverviewDetails?.count?.customerAccepted);
    const customerDenied = ensureNumber(LamiOverviewDetails?.count?.customerDenied);
    const totalCount = ensureNumber(LamiOverviewDetails?.count?.total);

    const netLost = ensureNumber(LamiOverviewDetails?.amount?.netLost);
                ...
                <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                    <Box >
                        <Typography variant="h4" sx={{ textTransform: 'uppercase' }} gutterBottom>
                            {LamiOverviewDetails?.name}
                        </Typography>
                        <Typography variant="subtitle2">
                            Designation:  {LamiOverviewDetails?.designation}
                        </Typography>
                    </Box>
                    ...
                    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "left", gap: "5px" }} >
                        <Box sx={{ display: "flex", gap: "5px" ,justifySelf:"right" }}>
                            ...
                            <Typography variant="h4" color="red" gutterBottom>
                                {LamiOverviewDetails?.totalCoureir}
                            </Typography>
                        </Box>
                        <Box sx={{ display: "flex", gap: "5px" ,justifySelf:"right"  }}>
