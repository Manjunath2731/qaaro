...
import DataPanel from 'src/Users/LamiUser/Courier/CourierHistory/DataPannel';
import OffsetForm from 'src/Users/LamiUser/Courier/CourierHistory/OffsetForm';
import { fetchDepoOverviewDetails } from 'src/slices/DepoClient/DepoPieChart';
...
interface DepoOverviewDetailsProps {
    depoAdminId: string;
}

const DepoPieChart: React.FC<DepoOverviewDetailsProps> = ({ depoAdminId }) => {
    ...
    useEffect(() => {
        dispatch(fetchDepoOverviewDetails(depoAdminId));
    }, [dispatch, depoAdminId]);

    const DepoOverviewDetails = useSelector((state: RootState) => state.depoOverviewDetails.details);

    const ensureNumber = (value: any) => typeof value === 'number' ? value : parseFloat(value) || 0;

    const total = ensureNumber(DepoOverviewDetails?.value?.total);
    const open = ensureNumber(DepoOverviewDetails?.value?.open);
    const success = ensureNumber(DepoOverviewDetails?.value?.success);
    const lost = ensureNumber(DepoOverviewDetails?.value?.lost);
    const returnedCount = ensureNumber(DepoOverviewDetails?.value?.returned);

    console.log("Values - Total:", total, "Open:", open, "Success:", success, "Lost:", lost, "Returned:", returnedCount);

    const openCount = ensureNumber(DepoOverviewDetails?.count?.open);
    const returned = ensureNumber(DepoOverviewDetails?.count?.returned);
    const customerAccepted = ensureNumber(DepoOverviewDetails?.count?.customerAccepted);
    const customerDenied = ensureNumber(DepoOverviewDetails?.count?.customerDenied);
    const totalCount = ensureNumber(DepoOverviewDetails?.count?.total);

    const netLost = ensureNumber(DepoOverviewDetails?.amount?.netLost);
    ...
    return (
        <Card sx={{ ml: "20px", mr: "20px", mb: "20px" }}>

            <Box p={{ xs: 2, md: 4 }}>
                <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                    <Box >
                        <Typography variant="h4" sx={{ textTransform: 'uppercase' }} gutterBottom>
                            {DepoOverviewDetails?.name}
                        </Typography>
                        <Typography variant="subtitle2">
                            Designation:  {DepoOverviewDetails?.designation}
                        </Typography>
                    </Box>

                    <Box sx={{ display: "flex", flexDirection: "column", alignItems: "left", gap: "5px" }} >
                        <Box sx={{ display: "flex", gap: "5px", justifySelf: "right" }}>
            ...
            <DataPanel onClose={handleClose} />
            <OffsetForm open={isOffsetOpen} onClose={handleOffsetClose} />

        </Card>
    );
}

export default DepoPieChart;
