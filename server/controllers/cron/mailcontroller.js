...
async function processMail(lamilist) {
    ...
    // Helper function to connect to the IMAP server with retries
    async function connectToImap(config, retries = 3) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                const connection = await imaps.connect(config);
                return connection;
            } catch (error) {
                console.error(`Attempt ${attempt} - Failed to connect to IMAP server: ${error.message}`);
                if (attempt === retries) {
                    throw new Error('Failed to connect to IMAP server after multiple attempts');
                }
            }
        }
    }
    ...
    const promises = lamilist.map(async lami => {
        await LamiAccountModel.findByIdAndUpdate(
            { _id: lami._id },
        ...
        try {
            ...
            const searchCriteria = ['ALL', ['SINCE', last_run]];
            const fetchOptions = {
                bodies: ['HEADER', 'TEXT', ''],
            };
                            ...
                            if (edata.attachments?.attachments?.length > 0) {
                                edata.attachments?.attachments?.forEach(element => {
                                    ...
                                    const params = {
                                        ...
                                        Body: Buffer.from(element?.content),
                                        ContentType: element?.contentType,
                                        ContentDisposition: 'inline',
                                        ContentId: element?.contentId,
                                        Metadata: {
